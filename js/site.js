/*
  檔案功能：site.js
  被引用目的：為靜態 HTML 頁面加入互動層，讓導覽選單、動畫狀態、手風琴與可及性屬性能依使用者操作而改變。
  編寫安排：等待 DOMContentLoaded 後才執行，確保 HTML 結構已載入；再使用 querySelector 取得元素，透過 classList 切換 CSS class，讓 JavaScript 控制狀態、CSS 負責視覺。
  與 HTML/CSS 的關係：HTML 提供 .site-header-toggle、.site-menu、.accordion-item 等 class；CSS 定義 .is-open、.is-expanded、.js-reveal、.is-visible 的樣式；本檔負責在適當時機加入或移除這些 class。
*/
...
// ==========================================================================
// 全域獨立函式：處理展覽日誌與手風琴項目的 +/- 切換
// 備註：此函式必須放在 DOMContentLoaded 外層，確保 HTML 內聯 onclick 屬性能直接呼叫。
// ==========================================================================
function toggleAccordion(header) {
  // 向上尋找最近的 accordion-item 外殼容器
  var item = header.closest('.accordion-item');
  if (!item) return;

  // 取得內部的符號元素 (+ 或 -)
  var icon = item.querySelector('.accordion-toggle-icon');
